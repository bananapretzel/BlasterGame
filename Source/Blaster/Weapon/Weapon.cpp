...
AWeapon::AWeapon() {
	...
	// Giving the weapon physics so it can bounce off stuff when dropped etc
	WeaponMesh->SetCollisionResponseToAllChannels(ECollisionResponse::ECR_Block);
	// Making the pawn not collide with it so they can walk through it etc
	WeaponMesh->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Ignore);
	// Initial state before being first picked up
	WeaponMesh->SetCollisionEnabled(ECollisionEnabled::NoCollision);

	WeaponMesh->SetCustomDepthStencilValue(CUSTOM_DEPTH_BLUE);
	// Updating highlight value by forcing a refresh
	WeaponMesh->MarkRenderStateDirty();
	// All weapons will start with custom depth enabled
	EnableCustomDepth(true);

	AreaSphere = CreateDefaultSubobject<USphereComponent>(TEXT("AreaSphere"));
	AreaSphere->SetupAttachment(RootComponent);

	// Essentially disabling the sphere since detecting overlaps should be done on the server only.
	// In begin play collision will be enabled for the server only
